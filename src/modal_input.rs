use allegro::{Core, Bitmap, Color, Display, BitmapDrawingFlags, Flag};
...
impl ModalInput {
  pub fn new(core:&Core, display:&Display, w_width:i32, w_height:i32,inp_height:f32, is_readonly:bool, font:&Font,
      ) -> ModalInput {

        let rect = get_center_rect(w_width, w_height, (40.0 / 100.0) * (w_width as f32), 
    ...
    ModalInput {
      rect:rect.clone(),
      is_open: true,
      text_inp_rect: Rect::new(rect.x + padding_x,
        rect.y + padding_y, text_inp_width, inp_height),
      is_readonly,
  ...
  pub fn draw(&self, core:&Core, primitives:&PrimitivesAddon, font:&Font) {
    if self.is_open {
      ...
      primitives.draw_filled_rounded_rectangle(
        ...
        10.0, 10.0, Color::from_rgb(38, 36, 33));
    }
  }

  pub fn is_open(&self) -> bool { self.is_open }
  pub fn set_is_open(&mut self, is_open:bool) { self.is_open = is_open; }
