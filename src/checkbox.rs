...
impl CheckBoxGroup {
  pub fn new( label:&'static str, x:f32, y:f32, box_size:f32, texts: Vec<&'static str>, font:&Font) -> CheckBoxGroup {
    
    let (tw, th) = (font.get_text_width(label) as f32, 
      font.get_line_height() as f32);


    let mut curr_x = x;
    let mut group = CheckBoxGroup { label,components: Vec::new(),
    label_pos: (x, y)};
    for text in texts.into_iter() {
      group.components.push(CheckBox::new(
        curr_x, y + th + MARGIN, box_size, text
      ));
  ...
  pub fn draw(&self, core:&Core, primitives:&PrimitivesAddon, font:&Font) {
    core.draw_text(font, Color::from_rgb(154, 153, 153), 
    self.label_pos.0, self.label_pos.1, FontAlign::Left, self.label);
